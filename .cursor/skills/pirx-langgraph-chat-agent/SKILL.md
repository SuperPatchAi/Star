---
name: jai-langgraph-chat-agent
description: LangGraph 1.0+ stateful chat agent for J.Ai conversational AI interface. Covers agent graph structure, intent routing, 8 custom tools, RAG pipeline with pgvector semantic search, streaming via Vercel AI SDK and FastAPI WebSocket, AsyncPostgresSaver thread persistence, and J.Ai locked terminology enforcement. Use when building the J.Ai chat feature, agent nodes, tool definitions, embedding pipeline, or frontend chat integration.
---

# J.Ai LangGraph Chat Agent

## J.Ai Context

J.Ai users interact with their running data through a conversational interface. The chat agent is a **read-only observer** — it queries projections, drivers, training history, and insights but NEVER modifies projection state. Only the projection engine writes to projection state.
...
All → generate_response (J.Ai terminology enforcement) → END
```
...
3. LLM receives retrieved context + user question + J.Ai terminology system prompt
...
export function JaiChat() {
  const { messages, input, handleInputChange, handleSubmit, isLoading } = useChat({
    api: '/api/chat',
  });
  // Render messages with J.Ai dark-theme styling
}
```
...
```python
config = {"configurable": {"thread_id": f"jai-{user_id}-{conversation_id}"}}
